...
class SimpleSquareCamera:
    ...
    def __init__(self, params: SimpleSquareCameraParams):
        self.params = params
        self.initialize(self.params.map_path)

    def initialize(self, map_path):
        total_map = load_map(map_path)
        self.obstacles = total_map.obstacles
        self.size = self.obstacles.shape[:2]
        self.obstruction_map = load_or_create_shadowing(map_path)

    def computeView(self, position, attitude, half_length=None):
        if half_length is None:
            half_length = self.params.half_length
        view = np.zeros(self.size, dtype=bool)
        camera_width = half_length
