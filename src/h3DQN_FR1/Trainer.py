...
class H_DDQNTrainer:
    def __init__(self, params: H_DDQNTrainerParams, agent: H_DDQNAgent):
        ...
        if self.params.load_model != "":
            ...
            self.agent.load_weights_ll(self.params.load_model)
            self.agent.load_weights_hl(self.params.load_model)
    ...
    def train_agent(self):
        ...
        if self.params.batch_size > self.replay_memory_ll.get_size():
            return
        mini_batch = self.replay_memory_ll.sample(self.params.batch_size)

        self.agent.train_ll(mini_batch)

        # train high-level agent
        if self.is_h_terminated:
            if self.params.batch_size > self.replay_memory_hl.get_size():
                return
            mini_batch = self.replay_memory_hl.sample(self.params.batch_size)

            self.agent.train_hl(mini_batch)
