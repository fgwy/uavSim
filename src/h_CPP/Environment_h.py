...
class H_CPPEnvironment(BaseEnvironment):
    ...
    def step_h(self, state, random=False, exploit=False):
        ...
        if exploit:
            while state.goal_not_present():
                goal = self.agent.get_goal(state).reshape((self.agent.params.local_map_size, self.agent.params.local_map_size))
                ...
                if not valid:
                    ...
                    reward_l = self.rewards.calculate_reward_l(state, GridActions(action), next_state)
                    reward_h = self.rewards.calculate_reward_h(state, GridActions(action), next_state)
                    self.stats.add_experience((copy.deepcopy(state), action, reward_l, copy.deepcopy(next_state)))
            ...
            next_state = self.physics.step(GridActions(action))
            # TODO: separate reward based on hl-ll interaction
            reward_l = self.rewards.calculate_reward_l(state, GridActions(action), next_state)
            reward_h = self.rewards.calculate_reward_h(state, GridActions(action), next_state)
            self.trainer.add_experience_ll(state, action, reward_l, next_state)

            if state.goal_ultimated():
                self.trainer.add_experience_hl(state, action, reward_h, next_state)
                state.goal_active = False

            self.stats.add_experience((state, action, reward_l, copy.deepcopy(next_state)))
        self.step_count += 1
    ...
    def check_valid_target(self, target_lm, local_map):
        local_map = np.logical_not(local_map).astype(int)
        valid = target_lm*local_map[2]
        return not np.all(valid==0)
